...
namespace Tn
{
    trtNet::trtNet(const std::string& prototxt,const std::string& caffemodel, int class_num,const std::vector<std::string>& outputNodesName,
                    const std::vector<std::vector<float>>& calibratorData,RUN_MODE mode, int maxBatchSize)
    :mTrtContext(nullptr),mTrtEngine(nullptr),mTrtRunTime(nullptr),mTrtRunMode(mode),mTrtInputCount(0),mTrtIterationTime(0),mTrtBatchSize(maxBatchSize)
    {
        ...
        pluginFactorySerialize.class_num = class_num;
        ...
        mTrtPluginFactory.class_num = class_num;
    ...
    void trtNet::InitEngine()
    {
        ...
        std::cout << "mTrtBatchSize : " << mTrtBatchSize << std::endl;
...
//			std::cout << "dims : " << dims.nbDims << std::endl;
...
//		std::cout << "mTrtInputCount : " << mTrtInputCount << std::endl;
...
//            std::cout << "In doInference ...\n";
//            float *out = (float*) outputData;
//            std::cout << out[0] << out[1] << out[2] << std::endl;

            outputData = (char *)outputData + size;

//            float *out1 = (float*) outputData;
//            std::cout << out1[0] << out1[1] << out1[2] << std::endl;
        }
