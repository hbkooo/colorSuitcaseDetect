...
namespace Tn
{
    trtNet::trtNet(const std::string& prototxt,const std::string& caffemodel, int yolo_class_num,  const std::vector<std::string>& outputNodesName,
                    const std::vector<std::vector<float>>& calibratorData, int srcSize,RUN_MODE mode, int maxBatchSize)
    :mTrtContext(nullptr),mTrtEngine(nullptr),mTrtRunTime(nullptr),mTrtRunMode(mode),mTrtInputCount(0),mTrtIterationTime(0),mTrtBatchSize(maxBatchSize)
    {
        ...
        pluginFactorySerialize.yolo_class_num = yolo_class_num;
        pluginFactorySerialize.srcSize = srcSize;
        ...
        mTrtPluginFactory.yolo_class_num = yolo_class_num;
    ...
    :mTrtContext(nullptr),mTrtEngine(nullptr),mTrtRunTime(nullptr),mTrtRunMode(RUN_MODE::FLOAT32),mTrtInputCount(0),mTrtIterationTime(0)
    {
        ...
        InitEngine();
        std::cout << engineFile <<" is init " << std::endl;
    }
